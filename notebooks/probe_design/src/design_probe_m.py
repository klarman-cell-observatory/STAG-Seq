"""
design slidelock probes. By Dawn Chen and Qiyu Gong
"""
import os
...
def get_reference_paths(base_dir=None):
...
def save_design_probes_new(data_folder, probe_request, selected_probe_exon, const_3, const_5_1, const_5_2, save_top=3, filter_same_exon=True):
    data_list = []
    ...
    merged_df = all_probes.merge(probe_request[['gene_id', 'gene_symbol']], on='gene_id', how='left')
    # merged_df['gene symbol'].fillna('', inplace=True)
    merged_df['gene_symbol'] = merged_df['gene_symbol'].fillna('')
    ...
    if save_top > 0:
        ...
        df = selected
        df['gene_symbol_unique'] = df.groupby('gene_symbol').cumcount() + 1
        df['gene_symbol_unique'] = df['gene_symbol'] + '_' + df['gene_symbol_unique'].astype(str)
        df = df[['gene_symbol_unique'] + [col for col in df if col != 'gene_symbol_unique']]
        df = df.sort_values(['gene_symbol', 'gene_id','exon_rank', 'gene_symbol_unique'])
        # df = df.drop_duplicates(subset=['probe_5'], keep='first')
        df = df.groupby('gene_symbol').head(save_top).reset_index(drop=True)
